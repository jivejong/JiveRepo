...
- [Gemini](https://ai.google.dev) (`gemini-3.1-flash-lite`) for LLM report generation: structured JSON output, one
...
1. **Copy `.env.example` to `.env`** and fill in:
   ...
   - `GEMINI_API_KEY`: required to generate hero reports. Never commit it; `.env` is gitignored. `GEMINI_MODEL` optionally overrides the default model.
