...
It demonstrates enterprise Java idioms end to end: domain modeling with a state machine that actually enforces its transition table, Postgres full-text search, and structured LLM generation parsed against a JSON schema rather than scraped out of prose. Every feature was verified against real systems: the live Simpsons API, a real Postgres instance, real Gemini calls: and several of them behaved in ways that are easy to get wrong. Those findings are written up inline, next to the code they explain.
...
- Google Gemini API (`gemini-3.1-flash-lite`, via its OpenAI-compatible endpoint) for both AI features
...
3. **A Gemini API key**: free tier, from aistudio.google.com. Only the two AI features need it;
...
Then fill in your Postgres password and, if you want the AI features, your Gemini key. `config/local.yml` is gitignored; `config/local.yml.example` is the committed template. `application.yml` pulls it in
...
| `GEMINI_API_KEY` | _(empty)_                                    | Gemini API key: **never commit this** |

These override `config/local.yml`, so CI can inject credentials without a file on disk.

The Gemini key is bound into `GeminiProperties`; an unset key leaves the app running normally with `isConfigured()` false. Everything except the two AI endpoints works without one.
...
| `502` | The Simpsons API or Gemini is unreachable: the fault is upstream, not the caller's                                                                                                                |
...
Needs `GEMINI_API_KEY` (or `gemini.api-key` in `config/local.yml`).
...
generation actually happened. Not yet measured on `gemini-3.1-flash-lite` (the earlier ~830 figure was for a different model). Note Gemini counts its thinking tokens in `total_tokens` but not in `completion_tokens`, so the two will not sum.
...
Both features use `gemini-3.1-flash-lite`; an interview is 5-6 turns plus a closing assessment. Token cost has not been re-measured since the move from Groq's `gpt-oss` models.
...
It is verified by driving real Chromium with Playwright rather than by trusting that a build succeeded: `npm run verify`, `verify:flow` and `verify:offer` assert against the live DOM and the actual network calls made, against real Gemini generations. See [`frontend/README.md`](frontend/README.md).
...
74 tests across 10 classes. All but one are plain unit tests with mocked collaborators and no network: including the AI ones, which never call Gemini.
